...
if __name__ == "__main__":
    data_download = DataDownLoad(user='root', password='123456', host='127.0.0.1', database='meta')

    # Select statement without condition
    data_download.Select('common_config')
    # Insert
    data_download.Insert(table="common_config", values="'test_config_key', 'test_name', 'can_be_deleted', '2024-05-24 15:52'")
    # Update
    data_download.Update(table="common_config", set_values="value = 'can_be_deleted_2'", condition="config_key = 'test_config_key'")
    # Select statement with condition
    data_download.Select(table="common_config", condition="config_key = 'test_config_key'")

    data_download.close_connection()