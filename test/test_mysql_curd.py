...
if __name__ == "__main__":
    con = TestConnectionPool(user='root', password='123456', host='127.0.0.1', database='meta')

    # Select statement without condition
    con.Select('common_config')
    # Insert
    con.Insert(table="common_config", values="'test_config_key', 'test_name', 'can_be_deleted', '2024-05-24 15:52'")
    # Update
    con.Update(table="common_config", set_values="value = 'can_be_deleted_2'", condition="config_key = 'test_config_key'")
    # Select statement with condition
    con.Select(table="common_config", condition="config_key = 'test_config_key'")
